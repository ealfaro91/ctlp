...
class ProjectFsn(models.Model):
    ...
    @staticmethod
    def attach_signature_to_pdf(pdf_binary_base64, signature_image_base64):
        """ Method to attach a signature image to a PDF file. """
        ...
        # Crear un PDF con la imagen de firma
        packet = io.BytesIO()
        can = canvas.Canvas(packet, pagesize=letter)

        # Dibujar rectángulo blanco como fondo en la posición de la firma
        x, y, width, height = 400, 50, 120, 50
        can.setFillColor(colors.white)
        can.rect(x, y, width, height, fill=1, stroke=0)

        # Si quieres agregar texto negro, por ejemplo:
        # can.setFillColor(colors.black)
        # can.setFont("Helvetica", 10)
        # can.drawString(x + 5, y + height/2, "Firma:")

        # Dibujar la imagen de la firma encima del fondo blanco
        can.drawImage(ImageReader(io.BytesIO(signature_image)), x, y, width=width, height=height, mask='auto')
        can.save()

        # Mezclar el PDF original con el de la firma
        packet.seek(0)
        signature_pdf = PdfReader(packet)
        original_pdf = PdfReader(io.BytesIO(pdf_data))
        writer = PdfWriter()

        # Añadir la firma en la primera página
        first_page = original_pdf.pages[0]
        first_page.merge_page(signature_pdf.pages[0])
        writer.add_page(first_page)

        # Agregar el resto de páginas si las hay
    ...
    def _compute_approval_state_ids(self):
        ...
        for rec in self:
            rec.approval_log_ids  = rec.approva_log_ids.sorted(x=lambda x: x.sequence).mapped("role_id")
    ...
    def _compute_approval_state(self):
        ...
        for fsn in self:
            ...
            if not fsn.approval_log_ids:
              ...
              #  approved =
              #  fsn.approval_state_id = fsn.approval_state_ids.filtered(lambda x: x.role_id == fsn.approval_log_ids[-1]
                fsn.approved = True
